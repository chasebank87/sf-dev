...
function Invoke-UpdateTaskPasswords {
    ...
    $serversWithTasks = $sessionHelper.GetServersWithTasks()
    ...
    $allResults = Get-TasksFromAllServers -SessionInfos $sessionInfos -ServiceAccount $serviceAccount -SessionHelper $sessionHelper
    
    # Display results table
    Display-TaskSummary -Results $allResults -UserInteraction $UserInteraction -Logger $logger
    
    # Check if user wants to proceed
    if (-not (Confirm-PasswordUpdate -Results $allResults -Logger $logger)) {
        return
    }
    ...
    Update-TaskPasswordsOnAllServers -SessionInfos $sessionInfos -Results $allResults -ServiceAccount $serviceAccount -NewPassword $newPassword -SessionHelper $sessionHelper -UserInteraction $UserInteraction -Logger $logger
            ...
            if ($serverInfo.Tasks -and $serverInfo.Tasks.Count -gt 0) {
                foreach ($expectedTask in $serverInfo.Tasks) {
                    ...
                    if ($taskExists) {
                        ...
                        if (-not $isDynamic -and $expectedTask -notin $combinedTasks) {
